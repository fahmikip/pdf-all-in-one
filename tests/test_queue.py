...
def test_job_queue_lifecycle(tmp_path: Path) -> None:
    queue = JobQueue()
    first = queue.add(tmp_path / "one.pdf", "Compress PDF", tmp_path / "one_out.pdf")
    second = queue.add(tmp_path / "two.pdf", "Compress PDF", tmp_path / "two_out.pdf")
    ...
    queue.update(first.id, status=JobStatus.COMPLETED, progress=100)
...
def test_completed_job_cannot_be_cancelled(tmp_path: Path) -> None:
    queue = JobQueue()
    job = queue.add(tmp_path / "one.pdf", "Compress PDF", tmp_path / "out.pdf")
