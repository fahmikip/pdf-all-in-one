...
import pymupdf
import pytest
from core.pdf.watermark import (
    add_header_footer,
    add_image_watermark,
    add_page_numbers,
    add_text_watermark,
...
def test_text_watermark_and_page_numbers(sample_pdf: Path, tmp_path: Path) -> None:
    ...
    with pymupdf.open(numbered) as document:
...
def test_image_watermark_and_header_placeholders(sample_pdf: Path, tmp_path: Path) -> None:
    ...
    output = add_header_footer(
        marked, tmp_path / "header.pdf", {"header-left": "{filename}", "footer-right": "{page}/{pages}"}
    )
...
def test_place_image_on_selected_pages(sample_pdf: Path, tmp_path: Path) -> None:
    ...
    Image.new("RGB", (200, 100), (30, 120, 200)).save(photo, "JPEG")
    output = place_image(
        sample_pdf, tmp_path / "with_photo.pdf", photo, position="top-right", width_scale=0.4, page_range="1,3"
    )
    with pymupdf.open(output) as document:
...
def test_insert_objects_image_and_styled_text(sample_pdf: Path, tmp_path: Path) -> None:
    photo = tmp_path / "photo.jpg"
    Image.new("RGB", (120, 60), (10, 200, 30)).save(photo, "JPEG")
    items = [
        {"type": "image", "rect": [40, 40, 340, 190], "image": str(photo)},
        {
            "type": "text",
            "rect": [40, 210, 540, 270],
            "text": "Teks Biru Tebal",
            "font": "Arial",
            "size": 16,
            "color": [0, 90, 255],
        },
...
def test_insert_objects_missing_image(sample_pdf: Path, tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError):
        insert_objects(
            ...
            tmp_path / "x.pdf",
            page_index=0,
            items=[{"type": "image", "rect": [0, 0, 100, 50], "image": str(missing)}],
        )
