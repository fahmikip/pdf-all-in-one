...
class MainWindow(QMainWindow):
    def __init__(self, settings: Settings, config_store: ConfigStore) -> None:
        ...
        if isinstance(home, HomePage):
            home.tool_requested.connect(self.navigate)
    ...
    def _create_shortcuts(self) -> None:
        shortcuts = (
            ("Ctrl+Q", self.close),
            ("Ctrl+M", lambda: self.navigate("merge")),
            ("Ctrl+O", lambda: self.navigate("home")),
        )
    ...
    def navigate(self, page: str) -> None:
        ...
        if target is not None:
            ...
            if page in self.sidebar.buttons:
                self.sidebar.select(page) if not self.sidebar.buttons[page].isChecked() else None

    def check_updates_silently(self) -> None:
        if self.update_worker is not None:
            return
        worker = FunctionWorker(check_for_update, VERSION)
    ...
    def _show_update(self, result: object, *, manual: bool = False) -> None:
        ...
        elif clicked is visit:
            QDesktopServices.openUrl(QUrl(result.release_url))
        elif clicked is skip:
            self.settings.skipped_version = result.version
            self.config_store.save(self.settings)

    def _download_update(self, update: UpdateInfo) -> None:
        if self.update_worker is not None or update.installer is None:
            return
    ...
    def _update_progress(self, value: int, message: str) -> None:
        self._ensure_progress().setLabelText(message)
        self._ensure_progress().setValue(value)
        if value >= 100:
            self._ensure_progress().close()
