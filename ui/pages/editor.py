...
class EditorPage(QWidget):
    ...
    def _build_items(self) -> list[dict]:
        ...
        for item in self.scene.items():
            if not isinstance(item, (ImageItem, TextItem)):
                continue
            rect = item.sceneBoundingRect()
            coords = [
                round(rect.x() / self.zoom, 2),
                round(rect.y() / self.zoom, 2),
                round((rect.x() + rect.width()) / self.zoom, 2),
                round((rect.y() + rect.height()) / self.zoom, 2),
            ]
                ...
                items.append(
                    {
                        ...
                        "rect": coords,
                        "text": item.text,
                        "font": item._font.family(),
                        "size": round(item.size_pt, 1),
                        "color": _pt_color(item.color),
                    }
                )
        return items
