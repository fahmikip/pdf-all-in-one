    ...
    def start(self) -> None:
        ...
        if self.mode.currentText() == "Text to .txt":
            ...
            if not folder:
                return
            self.run_job(
                extract_images, folder, image_format=self.format.currentText().lower(), min_size=self.min_size.value()
            )
    ...
    def open_result(self) -> None:
        if getattr(self, "target", None):
            QDesktopServices.openUrl(
                QUrl.fromLocalFile(str(Path(self.target).parent if not Path(self.target).is_dir() else self.target))
            )
