...
class OrganizerPage(QWidget):
    def __init__(self) -> None:
        ...
        self.drop = DropZone()
        self.drop.setMaximumHeight(155)
        self.drop.choose_requested.connect(self.choose)
        self.drop.files_dropped.connect(self.open_files)
        ...
        self.grid.setGridSize(QSize(145, 190))
        self.grid.setResizeMode(QListWidget.ResizeMode.Adjust)
        self.grid.setMovement(QListWidget.Movement.Snap)
        self.grid.setDragDropMode(QListWidget.DragDropMode.InternalMove)
    ...
    def reset(self) -> None:
        if self.thumbnails:
            self._checkpoint()
            self._restore(OrganizerState(tuple(PageSpec(i) for i in range(len(self.thumbnails)))))
    ...
    def duplicate_selected(self) -> None:
        ...
        for index, row in enumerate(rows):
            spec = specs[row]
            self._insert_spec(row + 1 + index, spec)
        self._renumber()

    def _insert_spec(self, row: int, spec: PageSpec) -> None:
        pixmap = QPixmap.fromImage(self.thumbnails[spec.source_index])
        if spec.rotation:
            pixmap = pixmap.transformed(QTransform().rotate(spec.rotation), Qt.TransformationMode.SmoothTransformation)
    ...
    def rotate_selected(self, degrees: int) -> None:
        ...
        for item in self.grid.selectedItems():
            source, rotation = item.data(Qt.ItemDataRole.UserRole)
            item.setData(Qt.ItemDataRole.UserRole, (source, (rotation + degrees) % 360))
