...
class BatchPage(QWidget):
    def __init__(self, default_level: str = "recommended") -> None:
        ...
        add = QPushButton("Add PDFs"); add.setObjectName("ghost"); add.clicked.connect(self.add_files); folder = QPushButton("Output Folder"); folder.setObjectName("ghost"); folder.clicked.connect(self.choose_output); options.addWidget(add); options.addWidget(folder); options.addStretch(); layout.addLayout(options)
        ...
        controls = QHBoxLayout(); self.start_button = QPushButton("Start Queue"); self.start_button.setObjectName("success"); self.start_button.clicked.connect(self.start); self.pause_button = QPushButton("Pause Queue"); self.pause_button.setObjectName("warning"); self.pause_button.clicked.connect(self.toggle_pause); cancel = QPushButton("Cancel Task"); cancel.setObjectName("danger"); cancel.clicked.connect(self.cancel_selected); clear = QPushButton("Clear Completed"); clear.setObjectName("ghost"); clear.clicked.connect(self.clear_finished)
