...
class BatchPage(QWidget):
    def __init__(self, default_level: str = "recommended") -> None:
        ...
        self.output_label = QLabel("Output: next to each source file")
        self.output_label.setObjectName("muted")
        layout.addWidget(self.output_label)
        self.table = QTableWidget(0, 6)
        self.table.setHorizontalHeaderLabels(["Filename", "Task", "Status", "Progress", "Output", "Error"])
    ...
    def add_files(self) -> None:
        ...
        for filename in files:
            try:
                ...
                QMessageBox.warning(self, "Skipped PDF", str(exc))
                continue
            folder = self.output_dir or source.parent
            output = folder / f"{source.stem}_compressed.pdf"
            self.queue.add(source, "Compress PDF", output)
        self.refresh()
    ...
    def _run_next(self) -> None:
        ...
        worker = FunctionWorker(
            compress_pdf, job.source, job.output, self.level.currentText().lower(), with_progress=True
        )
        self.worker = worker
    ...
    def completed(self, job_id: str, result: object) -> None:
        ...
        if isinstance(result, CompressionResult):
            HistoryStore().add(
                job.source.name, "Batch Compress PDF", result.original_size, result.compressed_size, str(result.output)
            )
        self.refresh()
    ...
    def toggle_pause(self) -> None:
        ...
        self.pause_button.setText("Resume Queue" if self.paused else "Pause Queue")
        if not self.paused:
            self._run_next()
