...
class AboutPage(QWidget):
    def __init__(self) -> None:
        ...
        heading = QLabel("Tentang Developer")
        ...
        name = QLabel(DEVELOPER)
        name.setObjectName("title")
        card_layout.addWidget(name, alignment=Qt.AlignmentFlag.AlignHCenter)
        role = QLabel("Pengembang PDF Master")
        role.setObjectName("subtitle")
        card_layout.addWidget(role, alignment=Qt.AlignmentFlag.AlignHCenter)
        info = QLabel(f"PDF Master · Versi {VERSION} · Windows\nDibangun dengan Python & PySide6\n© 2026 {DEVELOPER}")
        info.setAlignment(Qt.AlignmentFlag.AlignCenter)
        info.setObjectName("muted")
        card_layout.addWidget(info)
        button = QPushButton("Kunjungi GitHub")
        button.setObjectName("primary")
        button.clicked.connect(lambda: QDesktopServices.openUrl(QUrl(GITHUB_URL)))
        card_layout.addWidget(button)
        privacy = QLabel("Privasi: file diproses secara lokal. Tanpa telemetri, analitik, atau unggahan dokumen.")
