...
class ConverterPage(QWidget):
    def __init__(self) -> None:
        ...
        mode_row = QHBoxLayout(); mode_row.addWidget(QLabel("Conversion")); self.mode = QComboBox(); self.mode.addItems(["Image to PDF", "PDF to JPG", "PDF to PNG", "PDF to WebP"]); self.mode.currentTextChanged.connect(self.mode_changed); mode_row.addWidget(self.mode, 1); layout.addLayout(mode_row)
    ...
    def mode_changed(self, mode: str) -> None:
        ...
        self.dpi.setEnabled(not to_pdf); self.pages.setEnabled(not to_pdf); self.quality.setEnabled(not to_pdf)
        self.status.setText("Add images to begin" if to_pdf else "Add one PDF to begin")

    def choose(self) -> None:
        if self.mode.currentText() == "Image to PDF":
            paths, _ = QFileDialog.getOpenFileNames(self, "Choose images", "", "Images (*.jpg *.jpeg *.png *.webp *.bmp *.tif *.tiff)")
        else:
            path, _ = QFileDialog.getOpenFileName(self, "Choose PDF", "", "PDF files (*.pdf)"); paths = [path] if path else []
            self.files.clear()
        self.add_paths(paths)

    def add_paths(self, paths: list[str]) -> None:
        to_pdf = self.mode.currentText() == "Image to PDF"
        valid = [path for path in paths if Path(path).is_file() and (Path(path).suffix.lower() in IMAGE_EXTENSIONS if to_pdf else Path(path).suffix.lower() == ".pdf")]
