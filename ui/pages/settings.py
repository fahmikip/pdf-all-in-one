...
class SettingsPage(QWidget):
    def __init__(self, settings: Settings, store: ConfigStore, on_check_updates: Callable[[], None] | None = None) -> None:
        ...
        self.output = QLabel(settings.default_output_folder or "Ask each time"); choose = QPushButton("Choose Folder"); choose.setObjectName("ghost"); choose.clicked.connect(self.choose_output); output_row = QHBoxLayout(); output_row.addWidget(self.output, 1); output_row.addWidget(choose); form.addRow("Default output", output_row)
        ...
        if on_check_updates is not None:
            check = QPushButton("Check Now"); check.setObjectName("ghost"); check.clicked.connect(on_check_updates); form.addRow("", check)
        save = QPushButton("Save Settings"); save.setObjectName("success"); save.clicked.connect(self.save); form.addRow(save); layout.addLayout(form)
        ...
        actions = QHBoxLayout(); refresh = QPushButton("Refresh Dependencies"); refresh.setObjectName("ghost"); refresh.clicked.connect(self.refresh); clear_temp = QPushButton("Clear Temporary Files"); clear_temp.setObjectName("ghost"); clear_temp.clicked.connect(self.clear_temp); clear_history = QPushButton("Clear History"); clear_history.setObjectName("danger"); clear_history.clicked.connect(lambda: HistoryStore().clear())
