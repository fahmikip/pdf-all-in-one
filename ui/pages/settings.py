...
class SettingsPage(QWidget):
    ...
    def refresh(self) -> None:
        ...
        self.dependencies.setRowCount(len(status))
        for row, (name, value) in enumerate(status.items()):
            self.dependencies.setItem(row, 0, QTableWidgetItem(name))
            self.dependencies.setItem(row, 1, QTableWidgetItem(value))
        self.dependencies.resizeColumnsToContents()

    def clear_temp(self) -> None:
        folder = local_data_dir() / "temp"
        count = 0
        if folder.exists():
