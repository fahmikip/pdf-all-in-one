    ...
    *,
    ...
    if not tesseract:
        raise OcrUnavailableError(
            "Tesseract OCR was not found. Install Tesseract or locate tesseract.exe in Settings."
        )
    ...
    if output.suffix.lower() != suffix:
        raise ValueError(f"OCR output must use {suffix}.")
...
def ocr_pdf(
    ...
    *,
    ...
    if output_format not in {"pdf", "txt"}:
        raise ValueError("OCR output format must be PDF or TXT.")
    if language not in available_languages(tesseract):
        raise ValueError(f"Tesseract language '{language}' is not installed.")
    with tempfile.TemporaryDirectory(prefix="pdfmaster-ocr-") as directory:
        work = Path(directory)
        ...
        with pymupdf.open(info.path) as document:
            for index, page in enumerate(document):
                ...
                if result.returncode:
                    raise RuntimeError(result.stderr.strip() or f"OCR failed on page {index + 1}.")
                if output_format == "pdf":
                    generated.append(base.with_suffix(".pdf"))
                else:
                    text_parts.append(base.with_suffix(".txt").read_text(encoding="utf-8", errors="replace"))
                if progress:
                    progress(round((index + 1) / info.pages * 95), f"OCR page {index + 1} of {info.pages}")
        if output_format == "pdf":
            if len(generated) == 1:
                with atomic_output(output) as temporary:
                    shutil.copyfile(generated[0], temporary)
            else:
                merge_pdfs(generated, output)
        else:
            if output.suffix.lower() != ".txt":
                raise ValueError("Text output must use .txt.")
            with atomic_output(output) as temporary:
