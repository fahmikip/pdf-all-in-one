...
class JobQueue:
    ...
    def cancel(self, job_id: str) -> bool:
        job = self.get(job_id)
        if not job or job.status not in {JobStatus.WAITING, JobStatus.PROCESSING}:
            return False
        self.update(job_id, status=JobStatus.CANCELLED)
        return True

    def clear_finished(self) -> None:
        with self._lock:
            self._jobs = [job for job in self._jobs if job.status in {JobStatus.WAITING, JobStatus.PROCESSING}]
