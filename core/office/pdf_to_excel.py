...
def _page_bands(page) -> list[tuple[float, str, object, float, float]]:
    ...
    for (_block, _line), words in groups.items():
            ...
            for _y0, kind, payload, _x0, _y1 in bands:
                if kind == "text":
                    ...
                    for table_row in payload:
                        if not any(value is not None and str(value).strip() for value in table_row):
                            continue
                        for column, value in enumerate(table_row):
                            ...
                            cell.alignment = WRAP
                            sheet.column_dimensions[_col_index(column)].width = min(
                                COLUMN_WIDTH,
                                max(
                                    sheet.column_dimensions[_col_index(column)].width or COLUMN_WIDTH,
                                    len(str(value)) * 1.35 + 3,
                                ),
                            )
                        row += 1
                    row += 1
                    ...
                    try:
                        ...
                        continue
            if progress:
                progress(round((page_index + 1) / pdf.page_count * 95), f"Exported page {page_index + 1}")
    if created == 0:
