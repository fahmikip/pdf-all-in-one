...
def office_to_pdf(
    source: str | Path, destination: str | Path, *, executable: str | Path | None = None, progress=None
) -> Path:
    libreoffice = find_libreoffice(executable)
    if not libreoffice:
        raise LibreOfficeUnavailableError(
            "LibreOffice is required for Office conversion. Install it or locate soffice.exe in Settings."
        )
    document = Path(source).resolve()
    output = Path(destination).resolve()
    if not document.is_file() or document.suffix.lower() not in OFFICE_EXTENSIONS:
        raise ValueError("Select a supported Word, Excel, or PowerPoint file.")
    if output.suffix.lower() != ".pdf":
        raise ValueError("Office conversion output must use .pdf.")
    if progress:
